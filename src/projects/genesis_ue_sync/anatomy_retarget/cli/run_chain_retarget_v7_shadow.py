"""Build authenticated two-beta V7 whole-chain shadow (femur axial + knee gate)."""
...
WHOLE_CHAIN_V7_MATRIX_KIND = "WholeChainRestFitMatrixV7"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite V7 matrix: {output}")
    ...
    try:
        ...
        if operator.runtime_digest(validate=False) != EXPECTED_OPERATOR_RUNTIME_DIGEST:
            raise ValueError("V7 matrix requires the frozen 142 operator")
        ...
        if oracle_sha != EXPECTED_ORACLE_SHA256:
            raise ValueError("V7 matrix requires the frozen Blender oracle")
        ...
        if capture_sha256s != FROZEN_CAPTURE_SHA256:
            raise ValueError("V7 matrix capture digests differ from the frozen pair")
        ...
        for label in ("213328", "213712"):
            ...
            if (
                ...
                raise ValueError(
                    f"V7 subject {label} failed automatic gates: "
            ...
            if "v7_femur_axial" not in method:
                raise ValueError(f"V7 requires femur-axial method tag, got {method}")
            if str(value.build_report.get("pose_map_composition", "")).find("right_multiply") < 0:
                raise ValueError("V7 keeps right_multiply pose composition")
            femur_scales = value.build_report.get("femur_length_scale", {})
            requested = value.build_report.get("femur_requested_skin_scale", {})
            for side, scale in dict(femur_scales).items():
                if abs(float(scale) - 1.0) > 1.0e-8:
                    raise ValueError(
                        f"V7 {side} applied femur scale must stay 1.0 under shared-rigid policy"
                    )
            ...
            subjects[label] = {
                ...
                "femur_length_scale": femur_scales,
                "femur_requested_skin_scale": requested,
                "terminal_hand_foot_mean_delta_by_pose": {
                    pose: cell["hand_foot_mean_delta"]
                    for pose, cell in terminal_report["cells"].items()
                },
                "knee_pose_containment_passed": True,
            }
        matrix = {
            "schema_version": 1,
            "artifact_kind": WHOLE_CHAIN_V7_MATRIX_KIND,
            "baseline_commit": BASELINE_COMMIT,
            "accepted_scope": "full_main_chain_shadow_v7",
            "source_operator_digest": operator.runtime_digest(validate=False),
        ...
        raise
    print(
        f"{WHOLE_CHAIN_V7_MATRIX_KIND} passed=true subjects=2 "
