"""Build authenticated two-beta V6 whole-chain shadow with hand/foot vs-142 hard gates."""
...
WHOLE_CHAIN_V6_MATRIX_KIND = "WholeChainRestFitMatrixV6"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite V6 matrix: {output}")
    ...
    try:
        ...
        if operator.runtime_digest(validate=False) != EXPECTED_OPERATOR_RUNTIME_DIGEST:
            raise ValueError("V6 matrix requires the frozen 142 operator")
        ...
        if oracle_sha != EXPECTED_ORACLE_SHA256:
            raise ValueError("V6 matrix requires the frozen Blender oracle")
        ...
        if capture_sha256s != FROZEN_CAPTURE_SHA256:
            raise ValueError("V6 matrix capture digests differ from the frozen pair")
        ...
        for label in ("213328", "213712"):
            ...
            if (
                ...
                raise ValueError(
                    f"V6 subject {label} failed automatic gates: "
            ...
            if str(value.build_report.get("pose_map_composition", "")).find("right_multiply") < 0:
                raise ValueError("V6 requires right_multiply pose composition")
            if "copy_142_terminal" not in str(value.build_report.get("terminal_hand_policy", "")):
                raise ValueError("V6 keeps copy_142 terminal hand rest/bind")
        ...
        matrix = {
            "schema_version": 1,
            "artifact_kind": WHOLE_CHAIN_V6_MATRIX_KIND,
            "baseline_commit": BASELINE_COMMIT,
            "accepted_scope": "full_main_chain_shadow_v6",
            "source_operator_digest": operator.runtime_digest(validate=False),
        ...
        raise
    print(
        f"{WHOLE_CHAIN_V6_MATRIX_KIND} passed=true subjects=2 "
