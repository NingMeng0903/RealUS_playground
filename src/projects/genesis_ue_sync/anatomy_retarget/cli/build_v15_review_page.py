...
def main() -> None:
    ...
    for manifest in sorted(base.glob('v15*/manifest.json')):
        ...
        for cell in data['cells']:
            for path in sorted((manifest.parent / cell / 'comparison').glob('*.png')):
                skin = manifest.parent / cell / 'smplx_skin' / 'rgb' / path.name
                entries.append(dict(experiment=manifest.parent.name, cell=cell,
                                    view=path.stem, path=os.path.relpath(path, output),
                                    skin_path=os.path.relpath(skin, output) if skin.is_file() else None))
    ...
    videos.sort(key=lambda item: ('translation_walk' not in item['name'], item['name']))
...
<p>修正前后使用同一 β、θ、SMPL-X 皮肤和相机，具体版本名称写在图内。双侧锁骨修正使走路、喝水的右臂及相连血管有明显位置变化，两份采集动作变化小；足部近景仍显示整段足骨与血管偏离目标脚的轮廓。局部关节和软组织尚未通过。</p>
...
<div id="skinpanel" hidden><p>同一相机的不透明 SMPL-X 皮肤，用于核对真实目标轮廓：</p><img id="skin" alt="同一相机的目标皮肤"></div>
...
function show(){const e=entries.find(e=>e.experiment===experiment.value&&e.cell===cell.value&&e.view===view.value);if(!e)return;document.querySelector('#picture').src=e.path;const a=document.createElement('a');a.href=e.path;a.textContent='打开原图：'+e.experiment+' / '+e.cell+' / '+e.view;document.querySelector('#caption').replaceChildren(a);document.querySelector('#skinpanel').hidden=!e.skin_path;if(e.skin_path)document.querySelector('#skin').src=e.skin_path;}
