"""Build authenticated two-beta V8 whole-chain shadow (bone-first femur + gates)."""
...
from projects.genesis_ue_sync.anatomy_retarget.knee_pose_improve_v8 import (
    evaluate_knee_pose_improve_v8,
)
...
WHOLE_CHAIN_V8_MATRIX_KIND = "WholeChainRestFitMatrixV8"
...
def _anatomical_frame_gate(value) -> dict:
    ...
    for side in ("left", "right"):
        ...
        # Domains track mesh landmarks; allow a few mm vs controller segment target.
        if span_err > 0.008:
            failures.append(
                {
                    "side": side,
    ...
    return {
        ...
        "sides": sides,
        "failures": failures,
        "policy": "bone_first_anatomical_segment_frames",
        "publishable": False,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="chain_retarget_v7_node2_001 for knee-outside improvement gate",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite V8 matrix: {output}")
    ...
    try:
        ...
        if operator.runtime_digest(validate=False) != EXPECTED_OPERATOR_RUNTIME_DIGEST:
            raise ValueError("V8 matrix requires the frozen 142 operator")
        ...
        if oracle_sha != EXPECTED_ORACLE_SHA256:
            raise ValueError("V8 matrix requires the frozen Blender oracle")
        ...
        if capture_sha256s != FROZEN_CAPTURE_SHA256:
            raise ValueError("V8 matrix capture digests differ from the frozen pair")
        ...
        # SKEL-aligned containment objective uses the known flexed failure pose.
        ...
        for label in ("213328", "213712"):
            ...
            terminal_report = evaluate_terminal_pose_regression_v6(
                value,
                pose_map,
                asset=asset,
                smplx_model=model,
            ...
            improve_report = evaluate_knee_pose_improve_v8(
                value,
                pose_map,
                asset=asset,
                smplx_model=model,
            ...
            hard_reports = {
                **reports,
                ...
                "knee_pose_improve_v8": improve_report,
                "anatomical_frame_gate_v8": frame_report,
            }
            if (
                ...
                raise ValueError(
                    f"V8 subject {label} failed automatic gates: "
            ...
            if "v8_bone_first_femur" not in method:
                raise ValueError(f"V8 requires bone-first method tag, got {method}")
            if str(value.build_report.get("pose_map_composition", "")).find("right_multiply") < 0:
                raise ValueError("V8 keeps right_multiply pose composition")
            ...
            for side, scale in dict(femur_scales).items():
                if not 0.97 <= float(scale) <= 1.03:
                    raise ValueError(f"V8 {side} femur scale out of bounds: {scale}")
            ...
            subjects[label] = {
                ...
                "knee_pose_improve_v8_passed": True,
                "worst_outside_improve_m": improve_report["cells"]["pose_213328"][
                    "worst_outside_improve_m"
                ],
            }
        matrix = {
            "schema_version": 1,
            "artifact_kind": WHOLE_CHAIN_V8_MATRIX_KIND,
            "baseline_commit": BASELINE_COMMIT,
            "accepted_scope": "full_main_chain_shadow_v8",
            "source_operator_digest": operator.runtime_digest(validate=False),
        ...
        raise
    print(
        f"{WHOLE_CHAIN_V8_MATRIX_KIND} passed=true subjects=2 "
