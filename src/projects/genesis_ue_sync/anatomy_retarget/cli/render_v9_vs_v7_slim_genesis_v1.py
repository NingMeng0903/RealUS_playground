"""Slim Genesis compare: V7 vs V9 seat+inside embed (overlay/outside/full_anatomy)."""
...
# Overlay = bones through translucent SMPL-X; outside = poke heatmap;
# full_anatomy = Blender soft tissue (organs/vessels) for explosion check.
KEEP_CAMERAS = (
    "left_knee_ap",
    "left_knee_lateral",
    "right_knee_ap",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--v9-shadow", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--keep-set",
        type=Path,
        default=Path(
            "outputs/anatomy_retarget/v8_candidates/review_slim_20260803/v9_vs_v7"
        ),
    )
    parser.add_argument("--backend", default="cuda")
    parser.add_argument(
        "--delete-full-after-slim",
        action="store_true",
        help="After extracting slim keep cameras, delete full per-layer RGB trees",
    )
    return parser
...
def _render_shadow(
    *,
    ...
    subject_path = shadow / subject_subdir
    if not subject_path.is_dir():
        # Smoke path may store the subject at the shadow root.
        subject_path = shadow
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "V9VsV7SlimGenesisCompare",
        "subject": "213328",
        "pose": "pose_213328",
        "publishable": False,
        "review_layers": {
    ...
    for label, path in (
        ...
        ("v9", args.v9_shadow.resolve()),
    ):
        report["shadows"][label] = _render_shadow(
            label=label,
            shadow=path,
            subject_subdir="subject_213328",
            output=output,
            operator=operator,
            calibration=calibration,
            model=model,
    ...
    if args.delete_full_after_slim:
        for label in ("v7", "v9"):
            ...
            if full.exists():
                shutil.rmtree(full)
    print(f"V9VsV7SlimGenesisCompare -> {output}")
