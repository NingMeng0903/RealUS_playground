...
def apply_source_skin_volume_registration(
    ...
    mapped, section_report = _section_residual_regularizer(
        asset,
        query,
        mapped,
        skin_vertices + skin_delta,
        target_vertices,
        target_faces,
    )
    ...
    for mesh_name, (start, stop), tissue in zip(
        ...
        refined, barrier_report = surface_barrier_refine(
            ...
            target_vertices,
            target_faces,
            strain_reference_vertices=query[start_i:stop_i],
        )
        mapped[start_i:stop_i] = refined
