    ...
    *,
    ...
    for mesh_name, tissue, (start, stop) in zip(
        ...
        refined, strain_report = limit_edge_strain(
            ...
            maximum_ratio=1.08 if is_tube else 1.12,
            iterations=100 if is_tube else 300,
        )
        barrier_report = None
        if (
            is_tube
            and surface_vertices is not None
            and surface_faces is not None
        ):
            refined, barrier_report = surface_barrier_refine(
                ...
                strain_reference_vertices=reference[start_i:stop_i],
            )
