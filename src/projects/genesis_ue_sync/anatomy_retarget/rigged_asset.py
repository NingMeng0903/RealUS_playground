...
class AnatomyRiggedAsset:
    ...
    source_influence_offsets: np.ndarray | None = None
    source_influence_group_indices: np.ndarray | None = None
    source_influence_values: np.ndarray | None = None
    source_group_names: list[str] | None = None
    source_group_mesh_indices: np.ndarray | None = None
    source_group_local_indices: np.ndarray | None = None
    source_group_bone_indices: np.ndarray | None = None
    runtime_driver_indices_compressed: np.ndarray | None = None
    runtime_driver_weights_compressed: np.ndarray | None = None
    ...
    # The hypothetical all-harmonic result is a reference coordinate system,
    # never the runtime bone mesh.  The three handles are sampled in that same
    # coordinate system and define station-wise material-fit residuals.
    harmonic_reference_vertices: np.ndarray | None = None
    harmonic_bone_head: np.ndarray | None = None
    harmonic_bone_mid: np.ndarray | None = None
    harmonic_bone_tail: np.ndarray | None = None
    # Pre-baked translation-only follow data.  Arrays are per vertex except
    # source_mesh_follow_modes, which is per source mesh.
    soft_follow_driver_indices: np.ndarray | None = None
    soft_follow_driver_weights: np.ndarray | None = None
    soft_follow_stations: np.ndarray | None = None
    ...
    def validate(self) -> None:
        ...
        audit_values = (
            self.source_influence_offsets,
            self.source_influence_group_indices,
            self.source_influence_values,
            self.source_group_names,
            self.source_group_mesh_indices,
            self.source_group_local_indices,
            self.source_group_bone_indices,
        )
        if any(value is not None for value in audit_values):
            if not all(value is not None for value in audit_values):
                raise ValueError("authoritative source influence audit must be stored as one set")
            offsets = np.asarray(self.source_influence_offsets, dtype=np.int64)
            group_indices = np.asarray(self.source_influence_group_indices, dtype=np.int64)
            influence_values = np.asarray(self.source_influence_values, dtype=np.float64)
            group_count = len(self.source_group_names or [])
            if offsets.shape != (len(vertices) + 1,) or offsets[0] != 0:
                raise ValueError("source_influence_offsets must be CSR offsets for every vertex")
            if int(offsets[-1]) != len(group_indices) or len(group_indices) != len(influence_values):
                raise ValueError("source influence CSR arrays have inconsistent lengths")
            if group_indices.size and (
                int(group_indices.min()) < 0 or int(group_indices.max()) >= group_count
            ):
                raise ValueError("source influence group index is invalid")
            for name in (
                "source_group_mesh_indices",
                "source_group_local_indices",
                "source_group_bone_indices",
            ):
                if np.asarray(getattr(self, name)).shape != (group_count,):
                    raise ValueError(f"{name} must have one entry per source group")
        ...
        if self.source_bone_names is not None:
            ...
            if any(value is not None for value in fk_metadata):
                if not all(value is not None for value in fk_metadata):
                    raise ValueError("source-rig FK metadata must be stored as one complete set")
                for name, value in (
                    ("source_bone_roll", self.source_bone_roll),
                    ("source_bone_use_connect", self.source_bone_use_connect),
                    ("source_bone_inherit_scale", self.source_bone_inherit_scale),
                ):
                    if np.asarray(value).shape != (bone_count,):
                        raise ValueError(f"{name} must be [source_bone_count]")
            ...
            for name in ("harmonic_bone_head", "harmonic_bone_mid", "harmonic_bone_tail"):
                value = getattr(self, name)
                if value is not None and np.asarray(value).shape != (bone_count, 3):
                    raise ValueError(f"{name} must be [source_bone_count, 3]")

        vertex_count = len(vertices)
        if self.harmonic_reference_vertices is not None and np.asarray(
            self.harmonic_reference_vertices
        ).shape != vertices.shape:
            raise ValueError("harmonic_reference_vertices must match vertices_rest")
        follow_arrays = (
            self.soft_follow_driver_indices,
            self.soft_follow_driver_weights,
            self.soft_follow_stations,
        )
        if any(value is not None for value in follow_arrays):
            if not all(value is not None for value in follow_arrays):
                raise ValueError("soft-follow indices, weights and stations must be stored together")
            follow_i = np.asarray(self.soft_follow_driver_indices, dtype=np.int32)
            follow_w = np.asarray(self.soft_follow_driver_weights, dtype=np.float64)
            follow_s = np.asarray(self.soft_follow_stations, dtype=np.float64)
            if follow_i.shape != follow_w.shape or follow_i.shape != follow_s.shape:
                raise ValueError("soft-follow indices, weights and stations must have equal shapes")
            if follow_i.ndim != 2 or follow_i.shape[0] != vertex_count:
                raise ValueError("soft-follow arrays must be [vertex_count, influence_count]")
            if self.source_bone_names and follow_i.size and (
                int(follow_i.min()) < 0 or int(follow_i.max()) >= len(self.source_bone_names)
            ):
                raise ValueError("soft_follow_driver_indices contains an invalid bone")
            if not np.all(np.isfinite(follow_w)) or np.any(follow_w < 0.0):
                raise ValueError("soft_follow_driver_weights must be finite and non-negative")
            active = follow_w.sum(axis=1) > 1.0e-8
            if np.any(active) and not np.allclose(
                follow_w[active].sum(axis=1), 1.0, atol=1.0e-5, rtol=0.0
            ):
                raise ValueError("active soft-follow weights must sum to one")
            if not np.all(np.isfinite(follow_s)):
                raise ValueError("soft_follow_stations contains non-finite values")
        for name in ("soft_follow_strength", "soft_component_ids"):
            value = getattr(self, name)
            if value is not None and np.asarray(value).shape != (vertex_count,):
                raise ValueError(f"{name} must have one value per vertex")
        if self.source_mesh_follow_modes is not None and len(self.source_mesh_follow_modes) != mesh_count:
            raise ValueError("source_mesh_follow_modes must have one entry per source mesh")
...
def _semantic_defaults(asset: AnatomyRiggedAsset) -> AnatomyRiggedAsset:
    ...
    if asset.source_bone_names is None:
        return asset
    legacy_modes = list(asset.source_bone_driver_types or [])
    semantic_a = (
        None
        if asset.source_bone_smplx_a is None
        else np.asarray(asset.source_bone_smplx_a, dtype=np.int16).copy()
    )
    semantic_b = (
        None
        if asset.source_bone_smplx_b is None
        else np.asarray(asset.source_bone_smplx_b, dtype=np.int16).copy()
    )
    ...
    if legacy_modes:
        ...
        for mode in legacy_modes:
            value = str(mode)
            if value in SOURCE_DRIVER_MODES:
                modes.append(value)
            elif value == "parent_follow":
                modes.append("bind_follow")
            elif value in {"direct_joint", "head_orientation"}:
                modes.append("joint_local")
            elif value.startswith("scapula_"):
                modes.append("rigid_group")
            elif "segment" in value or value.startswith(("knee_chain_", "foot_chain_")):
                modes.append("segment_root")
            else:
                modes.append("joint_local")
    ...
    if frame is None and asset.source_bone_smplx_a is not None and asset.source_bone_smplx_b is not None:
        ...
        for bone, mode in enumerate(legacy_modes):
            value = str(mode)
            if not value.startswith("scapula_"):
                continue
            side = "left" if value.endswith("left") else "right"
            required = (f"{side}_shoulder", f"{side}_collar", "spine3")
            if all(name in joint_id for name in required):
                frame[bone] = np.asarray([joint_id[name] for name in required], dtype=np.int16)
                if semantic_a is not None and semantic_b is not None:
                    semantic_a[bone] = int(joint_id[required[0]])
                    semantic_b[bone] = int(joint_id[required[1]])
    ...
    result = replace(
        ...
        source_bone_smplx_a=semantic_a,
        source_bone_smplx_b=semantic_b,
...
def save_rigged_asset(path: Path | str, asset: AnatomyRiggedAsset) -> Path:
    ...
    optional_payload = {
        "source_influence_offsets": asset.source_influence_offsets,
        "source_influence_group_indices": asset.source_influence_group_indices,
        "source_influence_values": asset.source_influence_values,
        "source_group_mesh_indices": asset.source_group_mesh_indices,
        "source_group_local_indices": asset.source_group_local_indices,
        "source_group_bone_indices": asset.source_group_bone_indices,
        "runtime_driver_indices_compressed": asset.runtime_driver_indices_compressed,
        "runtime_driver_weights_compressed": asset.runtime_driver_weights_compressed,
        "leg_material_coordinates": asset.leg_material_coordinates,
        "source_segment_coupling": asset.source_segment_coupling,
        "harmonic_reference_vertices": asset.harmonic_reference_vertices,
        ...
        "soft_follow_driver_indices": asset.soft_follow_driver_indices,
        "soft_follow_driver_weights": asset.soft_follow_driver_weights,
    ...
    if asset.source_mesh_follow_modes is not None:
        payload["source_mesh_follow_modes"] = np.asarray(
            asset.source_mesh_follow_modes, dtype=object
        )
    if asset.source_group_names is not None:
        payload["source_group_names"] = np.asarray(asset.source_group_names, dtype=object)
    if asset.source_bone_names is not None:
        ...
        if asset.source_bone_roll is not None:
            payload.update(
...
def load_rigged_asset(path: Path | str, *, validate: bool = True) -> AnatomyRiggedAsset:
    ...
    target_local = (
        np.asarray(data["target_bind_local"], dtype=np.float32)
        if "target_bind_local" in data.files
        else np.array(source_local, copy=True)
    )
    target_global = (
        np.asarray(data["target_bind_global"], dtype=np.float32)
        if "target_bind_global" in data.files
        else np.array(source_global, copy=True)
    )
    ...
    target_head = (
        _points_from_bone_local(data["target_bone_head_local"], target_global).astype(np.float32)
        if "target_bone_head_local" in data.files
        else np.array(source_head, copy=True)
    )
    target_tail = (
        _points_from_bone_local(data["target_bone_tail_local"], target_global).astype(np.float32)
        if "target_bone_tail_local" in data.files
        else np.array(source_tail, copy=True)
    )
    asset = AnatomyRiggedAsset(
        ...
        source_influence_offsets=np.asarray(data["source_influence_offsets"], dtype=np.int64) if "source_influence_offsets" in data.files else None,
        source_influence_group_indices=np.asarray(data["source_influence_group_indices"], dtype=np.int32) if "source_influence_group_indices" in data.files else None,
        source_influence_values=np.asarray(data["source_influence_values"], dtype=np.float32) if "source_influence_values" in data.files else None,
        source_group_names=[str(v) for v in _string_array(data["source_group_names"]).tolist()] if "source_group_names" in data.files else None,
        source_group_mesh_indices=np.asarray(data["source_group_mesh_indices"], dtype=np.int32) if "source_group_mesh_indices" in data.files else None,
        source_group_local_indices=np.asarray(data["source_group_local_indices"], dtype=np.int32) if "source_group_local_indices" in data.files else None,
        source_group_bone_indices=np.asarray(data["source_group_bone_indices"], dtype=np.int32) if "source_group_bone_indices" in data.files else None,
        runtime_driver_indices_compressed=np.asarray(data["runtime_driver_indices_compressed"], dtype=np.int16) if "runtime_driver_indices_compressed" in data.files else None,
        runtime_driver_weights_compressed=np.asarray(data["runtime_driver_weights_compressed"], dtype=np.float32) if "runtime_driver_weights_compressed" in data.files else None,
        source_bone_names=(
        ...
        source_segment_coupling=np.asarray(data["source_segment_coupling"], dtype=np.float32).reshape(-1, 4, 4) if "source_segment_coupling" in data.files and data["source_segment_coupling"].size else None,
        harmonic_reference_vertices=np.asarray(data["harmonic_reference_vertices"], dtype=np.float32).reshape(-1, 3) if "harmonic_reference_vertices" in data.files and data["harmonic_reference_vertices"].size else None,
        harmonic_bone_head=np.asarray(data["harmonic_bone_head"], dtype=np.float32).reshape(-1, 3) if "harmonic_bone_head" in data.files and data["harmonic_bone_head"].size else None,
        harmonic_bone_mid=np.asarray(data["harmonic_bone_mid"], dtype=np.float32).reshape(-1, 3) if "harmonic_bone_mid" in data.files and data["harmonic_bone_mid"].size else None,
        harmonic_bone_tail=np.asarray(data["harmonic_bone_tail"], dtype=np.float32).reshape(-1, 3) if "harmonic_bone_tail" in data.files and data["harmonic_bone_tail"].size else None,
        soft_follow_driver_indices=np.asarray(data["soft_follow_driver_indices"], dtype=np.int16) if "soft_follow_driver_indices" in data.files else None,
        soft_follow_driver_weights=np.asarray(data["soft_follow_driver_weights"], dtype=np.float32) if "soft_follow_driver_weights" in data.files else None,
        soft_follow_stations=np.asarray(data["soft_follow_stations"], dtype=np.float32) if "soft_follow_stations" in data.files else None,
