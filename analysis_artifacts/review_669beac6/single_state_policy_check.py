...
def main():
    out = {
        ...
        'band_ablation_warning': '0.5 N is an offline force-direction deadband ablation, not a force bound and not deployed.',
        'feature_changed': False,
        'episode_assumption': 'Each point starts a fresh solver with a full repair allowance. Historical logs omit exact measured angle. This does not replay the episode state of a continuous v8r2 scan.',
    ...
    for relative in ('peirastic/contact_qp/qp.py', 'peirastic/contact_qp/repair_policy.py', 'peirastic/contact_qp/repair_episode.py'):
    ...
    for name, ids in SELECTED.items():
        ...
        for cid in ids:
            ...
            point = dict(path=name, control_id=cid, elapsed_s=r['record_monotonic_s']-start,
                         ...
                         variants={})
            for label, cfg in (('v7', v7), ('v7_force_band_05_diagnostic_only', replace(v7, force_sign_band_n=.5)), ('v8', v8)):
