...
def main():
    ...
    if (HERE/'regression_metrics.json').exists():
        regression=json.loads((HERE/'regression_metrics.json').read_text())
        lines+=['','Old-seed shadow regression (701/702) below is a revisit of the initial failures, not independent acceptance.','',
            '| Seed | Policy | Status | Force RMSE N | Peak N | Angular rad | Acoustic bad mm |',
            '|---|---|---|---:|---:|---:|---:|']
        for r in sorted(regression,key=lambda r:(r['seed'],r['policy'])):
            lines.append('| '+' | '.join([str(r['seed']),r['policy'],r['status'],number(r,'rmse'),number(r,'force_peak_n'),number(r,'angular_travel_rad'),number(r,'acoustic_bad_path_m',1000)])+' |')
        regstart=json.loads((HERE/'regression_source_hashes_start.json').read_text())
        regend=json.loads((HERE/'regression_source_hashes_end.json').read_text())
        lines+=['',f'Regression source files changed during execution: `{json.dumps([k for k in regstart if regstart[k]!=regend.get(k)])}`.']
