...
for trial in s['trials']:
 for a in trial['attempts']:
  ...
  t=np.array([r['record_monotonic_s'] for r in controls]);start=(a['stages']['tracking_observed']-epoch)*1e-9;end=(a['stages']['path_done']-epoch)*1e-9
  ...
  result=dict(attempt=a['directory'],scan_s=end-start,segments={},selected_frames=[])
  with h5py.File(p/'raw.h5') as h:
   ...
   fig,axes=plt.subplots(3,3,figsize=(16,17));fig.suptitle(p.parent.name+' / '+p.name+' — saved grayscale, confidence, shallow column confidence')
   for j,(label,index) in enumerate(selected):
    ...
    axes[j,2].set_title(f'Shallow means delta R-L={fq[2]-fq[0]:.3f}, deadband=.10')
   fig.tight_layout();fig.savefig(OUT/(p.parent.name+'.png'),dpi=120);plt.close(fig)
  results.append(result); print(a['directory'],json.dumps(result['segments']['late_half']),flush=True)
