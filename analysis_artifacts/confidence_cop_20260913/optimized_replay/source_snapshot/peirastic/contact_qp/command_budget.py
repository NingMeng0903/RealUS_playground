...
class _Pair:
    ...
    loading_source_w: float = 0.
    scan_source_w: float = 0.
...
class CommandBudget:
    ...
    def __init__(self,initial_j,capacity_j,stopping_reserve_j,*,max_command_interval_s,
        ...
        if task_power_source not in ('none','nominal_command','loading_scan_v1'):
            raise ValueError('explicit none or nominal_command task_power_source required')
        self.task_power_source=task_power_source
        self.assurance='two_port_command_model' if task_power_source!='none' else 'command_model'
        ...
        self._snapshot_loading_w=0.;self._snapshot_scan_w=0.
    ...
    def _settle(self,now_s,*,strict=False):
        ...
        if old is not None:
            ...
            # Proportional accounting splits consumption of the two authorized
            # sources; it is bookkeeping, not a new fusion/control weight.
            fraction=source_used_w/old.task_power_w if old.task_power_w>0 else 0.
            ...
            self.events.append(dict(event='logical_epoch_work',command_id=old.command_id,
                ...
                loading_source_available_w=old.loading_source_w,scan_source_available_w=old.scan_source_w,
                loading_source_used_j=dt*fraction*old.loading_source_w,
                scan_source_used_j=dt*fraction*old.scan_source_w,
    ...
    def snapshot(self,*,now_s,wrench_control_raw,rotation_base_tcp,nominal_twist_tool=None,
                 loading_twist_tool=None,scan_twist_tool=None):
        ...
        self._snapshot_loading_w=0.;self._snapshot_scan_w=0.
        if self.task_power_source=='loading_scan_v1':
            if nominal_twist_tool is not None:
                raise ValueError('loading_scan_v1 forbids mixed nominal authorization')
            loading=vector(loading_twist_tool,(6,),name='unmixed loading twist')
            scan=vector(scan_twist_tool,(6,),name='unmixed scan twist')
            if np.any(loading[[0,1,3,4,5]]!=0.) or np.any(scan[[2,4]]!=0.):
                raise ValueError('task authorization includes rocking or non-loading components')
            nominal=loading+scan
            with np.errstate(over='ignore',invalid='ignore'):
                loading_power=float(wrench @ loading);scan_power=float(wrench @ scan)
            if not math.isfinite(loading_power) or not math.isfinite(scan_power):
                raise ValueError('nonfinite loading/scan task power')
            self._snapshot_loading_w=max(0.,-loading_power)
            self._snapshot_scan_w=max(0.,-scan_power)
            task_power=self._snapshot_loading_w+self._snapshot_scan_w
            if not math.isfinite(task_power):raise ValueError('nonfinite loading/scan task power')
        elif loading_twist_tool is not None or scan_twist_tool is not None:
            raise ValueError('loading/scan components require loading_scan_v1')
        elif self.task_power_source=='nominal_command':
    ...
    def reserve(self,command_id,snapshot,final_velocity,*,now_s):
        ...
        self.pending=_Pair(command_id,fresh.wrench_environment,final,self._snapshot_rotation,
                           ...
                           loading_source_w=self._snapshot_loading_w,scan_source_w=self._snapshot_scan_w)
        self.started=False
    ...
    def commit(self,command_id,final_velocity,*,now_s,rotation_base_tcp,dual_success):
        ...
        self.events.append(dict(event='logical_epoch_commit',command_id=command_id,
            ...
            loading_source_available_w=pair.loading_source_w,scan_source_available_w=pair.scan_source_w,
