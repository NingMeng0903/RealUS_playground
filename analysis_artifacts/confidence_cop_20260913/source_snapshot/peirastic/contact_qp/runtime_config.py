...
def validate_study_config(source):
    ...
    return dict(mode=mode,configuration_valid=True,execution_policy=execution,
                ...
                energy_assurance='two_port_command_model' if (config.get('energy') or {}).get('task_power_source','none')!='none' else 'command_model',
                ...
                force_limit_assurance='measured_policy_not_prediction' if (config.get('qp') or {}).get('allocation_policy') in ('differential_repair_v8','confidence_angular_v1','confidence_cop_v1') else 'legacy')
