...
class PortInterval:
    ...
    dissipation_work_j: float = 0.

    def __post_init__(self):
        if isinstance(self.dissipation_work_j, (bool, np.bool_)) or not math.isfinite(self.dissipation_work_j) or self.dissipation_work_j < 0:
            raise ValueError("finite nonnegative dissipation work required")
...
class EnergyLedger:
    ...
    def settle(self, interval: PortInterval, *, now_s):
        ...
        try:
            ...
            port_work = upper*dt
            work = port_work + interval.dissipation_work_j
            upper += interval.dissipation_work_j / dt
        ...
        self.events.append(dict(event="settled", start_s=interval.start_s, end_s=interval.end_s,
                                work_upper_j=work, port_output_work_upper_j=port_work,
                                dissipation_work_j=interval.dissipation_work_j, total_charge_j=work,
                                released_j=released, balance_j=self.balance_j))
        return True
