...
def _exact_priority_candidate(qp, data, scaled_basis, scales, constraints, lo, hi,
                              *, preferred, arm, hard_rows):
    """Return the simultaneous zero-cost optimum only after strict admission.

    Epigraph cases retain the bounded numerical path. No public switch is
    needed: this is an algebraic proof of the same optimum, not a policy.
    """
    from .qp import _violation

    if constraints.shape[1] != 3:
        return None
    x = np.array([(data.loading_velocity_m_s + arm*data.visual_omega_target_rad_s)/scales[0],
                  data.visual_omega_target_rad_s/scales[1], preferred])
    if not np.isfinite(x).all():
        return None
    twist = scaled_basis @ x
    tolerance = min(qp.config.solver_tolerance, qp.config.feasibility_tolerance)
    if (_violation(constraints, lo, hi, x) > tolerance
            or _violation(*hard_rows, twist) > tolerance):
        return None
    if data.energy is not None and not data.energy.admissible(
            twist, tolerance_w=0., velocity_tolerance=0.):
        return None
    return x


def solve_priority(qp, data, scaled_basis, scales, cn, ln, un, *,
                   alpha_preferred, zero_path, deadline_s, diagnostics, hard_rows):
    ...
    diagnostics.update(priority_order=('progress', 'visual_omega', 'normal_pairing'),
        ...
        transparent=False, exact_priority_fast_path=False, visual_rows_active=False)
    exact_started = time.perf_counter()
    x = _exact_priority_candidate(qp, data, scaled_basis, scales, constraints, lo, hi,
                                 preferred=preferred, arm=arm, hard_rows=hard_rows)
    if x is not None:
        stages.extend(dict(stage=name, duration_s=time.perf_counter()-exact_started if index == 0 else 0.,
                           solver_status='exact_priority_optimum', accepted=True,
                           target=float(target), achieved=float(objective @ x))
                      for index, (name, objective, target, _) in enumerate(objectives))
        diagnostics.update(exact_priority_fast_path=True, iterations=0, numeric_attempts=[],
                           solver_status='exact_priority_optimum')
    else:
