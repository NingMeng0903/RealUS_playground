...
XY (and ωx/ωz) track the same 10×30 cm tool ellipse as cartesian_track.
Tool Z is force. Tool ωy is torque-tilt (CoP). Default F*=0 is air-safe;
tilt is contact-only and |e_f| only scales gain. On a surface set
FORCE_N to force.yaml (2 N).
...
def main() -> int:
    ...
    print(
        f"[MODE] HFPC ellipse  F*={FORCE_N:.1f}N  Z+ωy  "
