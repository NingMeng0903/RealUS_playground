...
Kinematics::Kinematics(const std::string& urdf_path, const Eigen::Matrix3d* tcp_R,
                       const Eigen::Vector3d* tcp_t)
  ...
  if (tcp_R != nullptr && tcp_t != nullptr) {
    model_.frames[tcp_id_].placement = pinocchio::SE3(*tcp_R, *tcp_t);
  }
