...
class RailExtensionTask:
    ...
    def __init__(
        ...
        *,
        ...
        # Escape direction latch (144217): freeze ±Y until σ clears escape band.
    ...
    def _latched_sigma_grad(
        ...
        *,
        ...
        sigma_brake_ref: float,
    ) -> float:
        """Hold escape sign from deep-σ until σ clears the early-avoidance band.

        Latch enter: σ < σ_brake (= sigma_ref, twist-brake threshold).
        Latch clear: σ > σ_escape (= sigma_ref × scale).
        Prevents ON/OFF left–right hunting when ∇σ and scan FF chatter.
        """
        g = float(sigma_grad_rail)
        s_now = float(sigma_min)
        s_esc = float(sigma_escape_ref)
        s_brake = float(sigma_brake_ref)
        if s_esc <= 1e-12 or not np.isfinite(s_now):
            return g
        if self._sigma_escape_latched:
            ...
            return abs(g) * sign
        if s_brake > 1e-12 and s_now < s_brake:
            sign = float(np.sign(g))
            if abs(sign) > 1e-12:
                self._sigma_escape_sign = sign
    ...
    def _capped_weight(self, w: float) -> float:
        """Hard cap: w ≤ w_max·(1+k_sigma_boost). Comments claimed this; code didn't."""
        w_cap = float(self.cfg.w_max) * (
            1.0 + max(float(self.cfg.k_sigma_boost), 0.0)
        )
        return float(min(max(float(w), 0.0), max(w_cap, 0.0)))
    ...
    def _call_reach(
        ...
        *,
        ...
        v_ff *= sig
        # σ-escape: latched direction + anti-oppose vs primary FF/reach.
        grad_used = self._latched_sigma_grad(
            float(sigma_grad_rail),
            sigma_min=float(sigma_min),
            sigma_escape_ref=float(sigma_escape_ref),
            sigma_brake_ref=float(sigma_brake_ref),
        )
        v_escape = float(self.cfg.k_esc) * (1.0 - sig) * float(grad_used)
    ...
    def __call__(
        ...
        *,
        ...
        sigma_scale : 1.0 when σ_min is healthy (``≥ sigma_escape_ref``),
            fades toward 0 at deep singularity.  Computed by the loop from
            early-avoidance onset (not raw σ_min).
        sigma_grad_rail : ``d σ_min / d y_rail`` under TCP-preservation.
        sigma_min / sigma_escape_ref / sigma_brake_ref : raw σ and hysteresis
            thresholds for escape-direction latch (optional; latch disabled
            when refs are ≤0).
