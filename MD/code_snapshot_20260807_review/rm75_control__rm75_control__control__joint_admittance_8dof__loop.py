...
def twist_scale_target(
    ...
    Default floor matches Jul-30 good era (4d15c1d / 7dde980).
    """
...
def sigma_escape_ref(
    sigma_ref: float,
    sigma_escape_ref_scale: float,
) -> float:
    """Early avoidance onset: σ_escape = σ_ref × scale (default 0.08×1.25=0.10)."""
    ref = max(float(sigma_ref), 0.0)
    scale = max(float(sigma_escape_ref_scale), 1.0)
    return ref * scale
...
class JointIkController:
    ...
    def update(
        ...
        *,
        ...
        # singularity. Hierarchy: σ_escape (earlier) arms rail/∇μ; σ_ref
        # brakes the primary twist (Jul-30 / 144217 review).
        sigma_ref = float(self.cfg.qp.sr_damping.sigma_ref)
        sigma_esc = sigma_escape_ref(
            sigma_ref,
            float(getattr(self.cfg.qp, "sigma_escape_ref_scale", 1.25)),
        )
        ...
        if (
            ...
            # Early avoidance at σ_escape (default 0.10); twist brake stays at σ_ref.
            sig_scale = 1.0
            if sigma_esc > 1e-9 and sigma_now < sigma_esc:
                sig_scale = max(sigma_now / sigma_esc, 0.25)
            ...
            v_ext, w_ext = self.rail_ext_task(
                ...
                dt_s=float(dt),
                sigma_min=sigma_now,
                sigma_escape_ref=sigma_esc,
                sigma_brake_ref=sigma_ref,
            )
            rail_ext_err = self.rail_ext_task.last_err_m
            if w_ext > 0.0:
                rail_task_vel = v_ext
                rail_task_weight = w_ext
            # Arm ∇μ starts at σ_escape so posture can change before twist brake.
            if sigma_esc > 1e-9 and sigma_now < sigma_esc:
