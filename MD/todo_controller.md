...
超声贴扫 `F~1 N` 时，**内环 `G_v` 可以按空中用**：隐藏伺服相对 1 N 仍是硬的，`K` 已经在 0.91–0.97 里当过扰动量。空中和接触的 `v/v_cmd` 差应小于这条增益散度，不必为「接触会另长一套延迟」再估一个 T0。  
这仍不是 `Y_a`。空中力环关着、F 是残差，Bode 里没有 `v/F`。接触后即使 `G_v` 不变，力口是 `Y_a=Y_v G_v S_f` 再串上组织 `K_e/s`。缺的是这条闭环，不是再测一遍 T0。硬垫 / 首触峰值 4–10 N 时「F 很小」不成立，那档不能抄空中 `G_v`。
