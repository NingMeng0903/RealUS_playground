...
* Bounce cut: force-rise impact arm + Teff press budget + SUSPECT_LOSS caps.
* Predictive retract stop + zero-centered retract brake on ``v_force``.
* Reverse interlock: no press restart while delayed TCP still retracts.
...
class AdmittanceConfig:
    ...
    # Block press restart while delayed TCP is still retracting.
    reverse_interlock_enter_m_s: float = 0.003
    reverse_interlock_exit_m_s: float = 0.001
    ...
    def from_dict(cls, raw: dict) -> AdmittanceConfig:
        ...
        return cls(
            ...
            reverse_interlock_enter_m_s=float(
                c.get("reverse_interlock_enter_m_s", 0.003)
            ),
            reverse_interlock_exit_m_s=float(
                c.get("reverse_interlock_exit_m_s", 0.001)
            ),
...
class AdmittanceController:
    ...
    def _press_vz_cap(self) -> float:
        """Press (+z) cap: recontact / suspect-recovery / low-force ceilings."""
        cap = self._v_z_cap()
        if (
            self._recontact_timer_s > 0.0
    ...
    def _admittance_z(
        ...
        *,
        ...
        # Rising edge of hold → short zero-centered brake on v_force.
        ...
        # Force-rise impact: arm on ḟ / F_pred, not delayed v_tcp sign.
        # Prefer fast-path ḟ; fall back to barrier ḟ when raw force absent.
        ...
        fast_rise = fdot_fast > float(cfg.impact_fdot_arm_n_s)
        predicted_overforce = (
            f_fast + max(fdot_fast, 0.0) * impact_pred_h
            > float(desired_force_n) + float(cfg.impact_fpred_over_n)
        )
        impact_want = (
            self._impact_rearm_ready
            and bool(physical_contact)
            and (fast_rise or predicted_overforce)
        )
        if impact_want:
            self._impact_arm_confirm_s += max(dt_eff, 0.0)
            if self._impact_arm_confirm_s + 1e-12 >= max(
                float(cfg.impact_arm_confirm_s), 0.0
            ):
                self._arm_impact_burst()
                self._impact_arm_confirm_s = 0.0
        else:
        ...
        if (
            ...
            < float(cfg.impact_rearm_f_frac) * abs(float(desired_force_n))
        ):
            self._impact_rearm_ready = True
        ...
        # Legacy continuous D_delay (sticky) — only if mode == always.
        d_delay_mag = 0.0
        ...
        # Impact burst: critical damping envelope (Di part of D0+Di target).
        d_impact_peak = self._critical_impact_delta(
            mass_z=mass_z, damping_ke=damping_ke
        )
        release = float(cfg.impact_damping_release_s)
        if self._impact_timer_s > release:
            d_impact = d_impact_peak
        elif self._impact_timer_s > 0.0 and release > 1e-9:
            d_impact = d_impact_peak * (self._impact_timer_s / release)
        else:
            d_impact = 0.0
        if (
            rising_edge
        ...
        # Retract brake: total D target → zero-centered extra (never × vr).
        d_base_preview = float(damping_ke)
        d_brake = 0.0
        ...
        if self._retract_brake_timer_s > 0.0 and brake_total > 0.0:
            d_brake_peak = max(0.0, brake_total - d_base_preview)
        ...
        # ΔD_hf: confirmed CONTACT chatter only.
        confirmed_contact = (
        ...
        # Zero-centered extras: smooth Di/HF; brake snaps on (passivity).
        d_base = float(damping_ke)
        d_extra_raw = float(d_delay + d_impact + damping_dimeas)
        bd_max = float(cfg.delay_damping_bd_max)
        if bd_max <= 0.0 and cfg.adaptive_ke.bd_max > 0.0:
            bd_max = float(cfg.adaptive_ke.bd_max)
        if bd_max > 0.0:
            d_extra_raw = min(d_extra_raw, max(0.0, bd_max - d_base))
        if (
            (rising_edge or self._impact_timer_s > release)
            and d_extra_raw > self._d_extra_smooth
        ):
            self._d_extra_smooth = d_extra_raw
        elif dt_eff > 0.0:
            if d_extra_raw >= self._d_extra_smooth:
                tau_d = max(float(cfg.var_damping_hf_attack_s), 0.01)
            else:
                tau_d = max(
                    min(
                        float(cfg.impact_damping_release_s),
                        float(cfg.var_damping_hf_release_fast_s),
                    ),
                    0.02,
                )
            blend = min(1.0, dt_eff / tau_d)
            self._d_extra_smooth += blend * (
                d_extra_raw - self._d_extra_smooth
            )
        else:
            self._d_extra_smooth = d_extra_raw
        d_extra = float(max(self._d_extra_smooth, 0.0) + d_brake)
        ...
        # Teff delay budget (press-only) — kills 80 mm/s slam after bounce.
        if bool(in_contact) or bool(physical_contact):
            press_cap = min(
                press_cap,
        ...
        if dt_eff <= 0.0:
            ...
            # Implicit Euler with zero-centered extras:
            #   (M/dt + D0 + D_extra) v+ = M/dt · v + D0 · v_r + drive
            # D_extra never multiplies v_r (passivity).
        ...
        # Direction-safe anti-windup (default off). Never weaken retract when
        # delayed TCP is still pressing into the surface.
        if (
            ...
            if v_z_cap > 0.0:
                ...
                hi = press_cap if press_cap > 0.0 else v_z_cap
                velocity = float(np.clip(velocity, lo, hi))
        # Direction interlock: wait for delayed TCP retract to end before press.
        ...
        if enter_v <= 0.0:
            ...
            if np.isfinite(v_tcp_filt):
                if float(v_tcp_filt) < -enter_v:
                    self._reverse_interlock = True
                elif float(v_tcp_filt) > -exit_v:
                    self._reverse_interlock = False
        if float(eff) > 0.0 and self._reverse_interlock:
            velocity = min(float(velocity), 0.0)
        self.reverse_interlock_active = bool(
            float(eff) > 0.0 and self._reverse_interlock
        )
