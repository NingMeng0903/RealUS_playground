...
* Continuous reverse-interlock blend (gate velocity from 50 ms regression).
* Press-side energy tank (force × Δx) scales active press only.
* Force-axis asymmetric slew (fast retract, soft press / zero-cross).
...
class AdmittanceConfig:
    ...
    reverse_interlock_enter_confirm_s: float = 0.020
    reverse_interlock_exit_confirm_s: float = 0.030
    v_gate_window_s: float = 0.050
    ...
    # Force-axis asymmetric slew [m/s²].
    ...
    def from_dict(cls, raw: dict) -> AdmittanceConfig:
        ...
        return cls(
            ...
            reverse_interlock_enter_confirm_s=float(
                c.get("reverse_interlock_enter_confirm_s", 0.020)
            ),
            reverse_interlock_exit_confirm_s=float(
                c.get("reverse_interlock_exit_confirm_s", 0.030)
            ),
            v_gate_window_s=float(c.get("v_gate_window_s", 0.050)),
...
class AdmittanceController:
    ...
    def _force_axis_slew(
        ...
        """Asymmetric slew: fast safety retract, soft press / zero-cross."""
        cfg = self.cfg
    ...
    def _admittance_z(
        ...
        *,
        ...
        # Press-side tank: scale only active press (retract unrestricted).
        u_press = max(
            drive + max(d_base, 0.0) * max(float(v_reference), 0.0),
            0.0,
        )
        ...
        drive = min(drive, 0.0) + float(gamma) * max(drive, 0.0)
        v_reference = (
        ...
        # Continuous reverse-interlock blend on gate velocity (50 ms regression).
        ...
        if enter_v <= 0.0:
            ...
            if np.isfinite(v_tcp_gate):
                ...
                gate = 0.0 if self._reverse_interlock else 1.0
            if self._reverse_interlock:
                gate = 0.0
        self.reverse_interlock_gate = float(np.clip(gate, 0.0, 1.0))
        if float(eff) > 0.0 and velocity > 0.0:
            velocity = float(velocity) * float(self.reverse_interlock_gate)
        self.reverse_interlock_active = bool(
            float(eff) > 0.0 and self.reverse_interlock_gate < 0.99
        )
