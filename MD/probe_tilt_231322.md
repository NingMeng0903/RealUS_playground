...
当前保持 `M=0.065`、`D=0.28`、`Fc=0.025 N·m`、`vmax=0.22 rad/s`、`amax=4.5 rad/s²`。正反转不采用不同增益。
...
优先用确认无外载、同样线缆布置下的多个静止姿态检查残余 My：近似常量更像偏置；随姿态变化更需检查载荷质量/重心和补偿。偏置原因明确后，如果仍需更细的随形，可以把 **0.020 N·m 作为下一轮单参数试验候选**，质量、阻尼和速度上限保持不变；本轮没有启用该候选，也没有用本次日志估计偏置后直接写入控制器。
