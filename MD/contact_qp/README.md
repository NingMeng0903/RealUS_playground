...
当前默认配置 `active_probe50_delay_kf_cop.yaml` 使用四变量外环
`[v_n, omega_y, alpha, sigma_I]`。有效 CoP 下，平移与转动采用硬等式
`v_n = v_m + c_p * (omega_y - omega_m)`，已删除 CoP 软代价及 `cop_weight`。
法向参考代价因此成为角向修正的机械代价，其等效角向权重为
`w_omega + w_v * (c_p * omega_s / v_s)^2`。未满足的超声请求由
`sigma_I` 表示；CoP 无效时省略等式。若等式与硬运动边界共同无解，
该次命令被拒绝，超声 slack 不放松配对。等式也导出到 TCP 约束，
由最终发布检查验证内层 IK 输出。该配置不使用能量罐。

原 IK 封装保持现有接口；声窗动作效果由真实闭环采集判断。以下文档
保留此前三变量外环与能量约束版本的历史说明。
