...
class LegacyForceWithTilt:
    ...
    def prepare(self, *, measurement_id: int, **kwargs) -> ForceOutput:
        zout = self.z_law.prepare(measurement_id=measurement_id, **kwargs)
        try:
            return self._combine(zout, kwargs, measurement_id=measurement_id)
        except Exception:
            self.z_law.abort()
