...
先给结论：你现在最有潜力的方向，不是再改一个 admittance 参数或加一个 DOB，而是做成：

> **基于真实位移功的异步多速率宏—微力控 + 接触可行性零空间控制。**

核心是把三件常被混在一起的事分开证明：

1. energy tank 保证物理端口无源；
2. force-band/接触 CBF 保证捕获接触后不脱离—重撞；
3. 最小干预 QP 保证约束不激活时，力精度与原控制器完全相同。

严格无源本身不保证“不弹跳”——一个无源但欠阻尼的系统照样会反复离开表面再撞回去。这一点应该成为论文的理论立足点。

## 对当前代码的判断

我检查了当前 `main`：

* 主循环和力反馈实际都是 200 Hz，不是 500 Hz；配置见 [joint_admittance_8dof.yaml](https://github.com/NingMeng0903/RealUS_playground/blob/main/rm75_control/configs/joint_admittance_8dof.yaml)。
* 当前法向控制是

  $$
  M\dot v+D(v-v_r)=F_d-F
  $$

  并由 `v_r` 积分追赶欠力。这个积分器、约 55 ms 的实际响应延迟以及退让饱和，正好形成你硬表面上的 2–3 Hz“欠力追入—超力退回—脱离—重捕获”循环。
* 控制器仍使用固定 15 ms 预测，并忽略传入的 `dt_actual`、`sensor_age_s` 和实际 TCP 速度，见 [controller.py](https://github.com/NingMeng0903/RealUS_playground/blob/main/rm75_control/rm75_control/control/admittance_common/controller.py)。
* 目前没有真正的 energy tank 状态和物理功记账。`proactive_force_ff` 中的 “energy-aware” 是启发式门控，不构成无源证明。
* 零空间中的 `-d qdot_prev` 是指令速度平滑，不是可测物理阻尼，不能作为 tank 收入，见 [secondary_composer.py](https://github.com/NingMeng0903/RealUS_playground/blob/main/rm75_control/rm75_control/control/joint_admittance_8dof/tasks/secondary_composer.py)。
* SR projector、QP slack 和跟踪误差会让“数学零空间”泄漏到末端；在纯 `qdot` 黑盒接口上，动力学零空间也不能自动变成物理能量解耦。

因此，当前版本最多能做稳定性和实验鲁棒性论证，不能严谨证明“物理端口无源且不弹跳”。

## 推荐控制架构

```mermaid
flowchart TD
    S["力传感器 + 微动台编码器<br/>500 Hz"] --> F["微动台法向力控<br/>唯一力积分器"]
    F --> Q["能量 + 接触约束 QP"]
    Q --> L["低质量法向微动台"]
    A["RM75 状态<br/>200 Hz"] --> C["切向轨迹 + 微动台居中"]
    C --> Q
    R["导轨 50 Hz<br/>工作空间调节"] --> C
```
...
* 低运动质量、高带宽电机；
* 输出端串联一段可标定的被动柔顺结构；
* 力传感器放在微动质量之后、尽可能靠近探头。

这一布置很关键。如果 F/T 传感器位于电机移动质量上游，它测到的是“接触力 + 微动台惯性力”，那你要么进行惯性补偿，要么无法仅用传感器力证明探头端口无源。

第一篇论文不建议同时增加主动切向自由度。切向摩擦使全 6D 一端口无源声明困难很多；切向超声/弹性成像激励更适合作为第二篇。

### 2. 500 Hz 微动台力环

唯一的力误差积分器放在微动台。RM75 不再同时积分法向力误差。

每个 2 ms 周期先得到高性能名义指令，再让一个很小的 QP 做最小修改：

* 第一优先级：能量不能透支；
* 第二优先级：\(F_{\min}\le F_n\le F_{\max}\)，且扫描时 \(F_{\min}>0\)；
* 第三优先级：微动台行程、速度、制动距离；
* 目标函数：尽可能保持原名义力指令。

这样约束不激活时，QP 输出严格等于名义控制器，因而可以给出“**nominal equivalence**”准确性定理。约束激活时不可能同时承诺任意环境下的精确力、严格无源和无限执行器能力；论文应声明最小必要降级，而不是声称永不牺牲。
...
$$
\Delta W_k \approx \bar F_k^\top (x_{k+1}-x_k)
$$

而不是 \(F^\top v_{\mathrm{cmd}}\Delta t\)。这使证明不依赖加速度跟踪，也不要求电流环力矩估计。

但真实位移是在动作之后才能测到，因此发出下一条命令前必须冻结一部分“已承诺但尚未结算的能量”：

$$
E_{\mathrm{free}}
=
E_{\mathrm{tank}}-E_{\mathrm{commit}}
\ge E_{\min}.
$$
...
* 下一次力/位置测量前可能产生的功；
* RM75 已发出但受 55 ms 延迟尚未完全体现的运动；
* 微动台制动距离；
* 零空间泄漏和传感器时间戳误差。

这比假设“机器人精确执行 \(qdot_d\)”强得多，而且只需要可实验标定的速度、延迟、位移和力上界。

## “不弹跳”应该怎样证明

不要把它塞进 passivity theorem。另立一个接触可行性定理：

* 接触状态只假定局部单调/扇区性质：继续压入会增大法向力；
* 在线估计或离线给出刚度区间 \([k_{\min},k_{\max}]\)；
* 由鲁棒 CBF 约束下一时刻仍处于力带内；
* 接触捕获和稳定扫描是两个 hybrid mode；
* 模式切换不重置 tank，使用无冲击的状态转移。

可以严谨声称：

> 在表面满足给定单调刚度区间、传感延迟有界且 QP 可行时，接触捕获后 \(F_n\ge F_{\min}>0\) 的集合前向不变，因此不会发生接触丢失—重捕获弹跳。

这比泛泛的“无源所以不弹”严谨得多。

## 粗细运动分工

* 微动台 500 Hz：力残差、高频接触变化、冲击吸收。
* RM75 200 Hz：切向位置、姿态、微动台慢速居中。
* 导轨 50 Hz：工作空间和低频姿态优化，不参与高频 passivation。

法向只允许微动台持有积分器。RM75 法向只能做微动台位置居中，例如把微动台保持在中间 40–60% 行程，不能再独立 chase force。

根据你测到的约 55 ms 延迟：若 RM75 接触法向速度仍允许 \(0.1\,\mathrm{m/s}\)，延迟期间潜在运动已经是 5.5 mm。微动台单边可用吸收行程至少应满足

$$
\Delta s_{\mathrm{reserve}}
\ge
\bar v_{a,n}T_d+
\frac{\bar v_f^2}{2a_{f,\max}}+
\Delta s_{\mathrm{unc}}.
$$

因此单边行程明显不能只做 2–3 mm；保留安全裕量后，总行程大致应在 14–20 mm 量级。或者把 RM75 接触模式法向速度限制到约 18 mm/s，此时延迟项约 1 mm。

## 高维零空间的正确作用

这是最可能形成理论新意的地方。

高维精确零空间不能瞬间“吸收末端能量”，因为它按定义不改变末端速度。对黑盒 `qdot` 机器人，关节自运动也不能被当作可回收阻尼功。

应该把零空间从“能量汇”重新定义成“**未来无源可行性的调节器**”：

* 最大化微动台剩余行程；
* 最大化法向制动和速度余量；
* 远离奇异位形、关节限位与碰撞；
* 减小已承诺延迟功的上界；
* 调节导轨和机械臂姿态，使下一周期 energy/contact QP 仍可行。

可以定义一个 passivity-viability margin，并给它加 CBF。理论贡献就是：

> 高维冗余不直接耗散能量，而是保持能量约束、接触约束和执行器约束的递归可行性。

这比 Ott 的“优先在 nullspace 中分配 passivity controller 阻尼”更适合你的纯速度黑盒平台。Ott 的方法以及 [Dietrich 等人的 nullspace energy tank](https://ieeexplore.ieee.org/document/7368097/)主要建立在可施加物理力矩/阻尼和可信动力学模型上。

不要为了论文打开动态零空间。没有力矩接口时，动力学投影漂亮但不能提供真实端口功证明。

## 论文漏洞总结

| 工作                                                                 | 有价值的部分                    | 对你场景的主要漏洞                                      |
| ------------------------------------------------------------------ | ------------------------- | ---------------------------------------------- |
| [Keemink 2018](https://doi.org/10.1177/0278364918768950)           | 黑盒速度环、phase-lead、低惯量整形    | 核心分析是一自由度 LTI；未解决 55 ms 延迟、多速率、零空间和内部柔性        |
| Samuel 2024 CDYOB                                                  | 高刚度接触下补偿黑盒机器人扰动           | 依赖近似解耦线性模型和名义逆；没有物理端口无源证明，模型/Q-filter 误差仍可注入能量 |
| Secchi 2019 tank-QP                                                | 离散时间 tank 与最小干预优化         | 假定实际速度近似等于期望速度；未覆盖黑盒延迟、异步采样和 nullspace 泄漏      |
| [Benzi 2021](https://doi.org/10.1109/ICRA48506.2021.9561098)       | 把 passivity 与 CBF 放进同一 QP | 500 Hz 实验依然假定底层速度跟踪可靠；不保证接触不丢失或力精度不降           |
| [De Stefano 2020](https://elib.dlr.de/134108/1/de_stefano_tro.pdf) | 对积分和纯延迟分别使用 TDPA          | 将真实机器人近似成“完美速度跟踪 + 整数纯延迟”；PC 耗散可能产生高频失真        |
| [Lee 2024 BEFM](https://doi.org/10.1109/TRO.2024.3443690)          | 多自由度、双向能流、参考轨迹作为第二端口      | 需要力矩接口和惯量估计；论文自己承认惯量误差、离散化、噪声和柔性下无源性不再保证       |

另一个容易忽略的问题是：恒力扫描本身是主动任务。若切向存在摩擦，机器人会持续向环境做功，有限预充 tank 不可能永远支付。正确表述应是：

* 法向接触端口严格无源，切向轨迹作为显式参考能量端口；或者
* 使用 Lee 式两端口 dissipativity，把参考轨迹供能明确写入 supply rate。
...
**Work-Credited Multi-Rate Macro–Micro Passivity and Viability Control for Black-Box Velocity Robots**

理论贡献：

1. 基于真实位移功、而非指令速度或电机电流的离散无源性；
2. 异步 500/200/50 Hz 事件驱动能量账本；
3. 未结算运动的 committed-energy reserve；
4. 约束不激活时的名义精度等价；
5. 高维 nullspace 对 passivity/contact QP 递归可行性的保证；
6. 无源性与接触不丢失的两个独立定理。

这是最接近 TRO/IJRR 级别的组合。单纯“energy tank + 新机械”不够新；“真实功记账 + 未结算功 + 可行性零空间 + 宏微结构”才形成完整理论主线。

实验上至少比较：当前控制、CDYOB、基于 commanded velocity 的传统 tank-QP、TDPA、你的 actual-work 方法、再加 viability nullspace。重点报告物理端口能量下界、接触丢失次数、力 RMSE、峰值力、微动台饱和率、QP 干预占空比和切向轨迹误差。只有证明能量约束绝大部分时间不激活且 RMSE 与名义控制一致，才能有力支持“安全不牺牲准确性”。
