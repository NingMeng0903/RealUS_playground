...
本节按\emph{依赖顺序}把整条设计说清楚：先有哪些硬约束，再为何要商空间、为何只消 J1、IRD 如何落到导轨一维、为何必须用法兰 9 维图卡、算子如何分层、用了哪些技巧，最后给出带 tip$\times$roll 姿态时\emph{必须按序}执行的旋转与查询步骤。后文各节给出对应公式与实现细节；此处保证思路连贯。
...
\subsection{为什么必须是 flange 上的 9 维嵌入}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{只商真对称}：仅 J1 yaw；probe45 roll 留在图卡。
...
旋转方向一致率 & \(99.97\%\) \\
独立网格准确率 & \(99.17\%\) \\
导轨 AD--FD 相对误差 & \(0.0098\%\) \\
TCP \(x\) AD--FD 相对误差 & \(0.0101\%\) \\
\hline\hline
\end{tabular}
\end{table}
