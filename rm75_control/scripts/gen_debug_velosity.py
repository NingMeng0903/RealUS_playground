...
def main() -> None:
    ...
    header = (
        ...
        "## 实现方法概览\n\n"
        "速度导纳（Velocity Admittance）栈：**笛卡尔速度下发**（`rm_movev_canfd`），"
        "外环 PBAC + 力导纳 sleeve 融合，内环无 WBC/QP。\n\n"
        "| 模块 | 文件 | 作用 |\n"
        "|---|---|---|\n"
        "| 主循环 | `hybrid_motion/loop.py` | 10ms CANFD 周期：读状态→φ 补偿力→外环 v_cmd→movev 下发 |\n"
        "| 外环控制器 | `hybrid_motion/controller.py` | PBAC 位置跟踪 + tool-Z 二阶导纳 + Dimeas 变阻尼 + sleeve 融合 |\n"
        "| 自适应阻尼 | `hybrid_motion/adaptive_ke.py` | ΔF/Δx EWMA → K̂_e → b_d = 2ζ√(mK̂_e) |\n"
        "| 力观测 | `hybrid_motion/observer.py` | φ 回归补偿 + 因果 LPF → f_ext |\n"
        "| 轨迹参考 | `hybrid_motion/reference.py` + `reference_shaper.py` | MotionReference 源 + 低通/整形 |\n"
        "| 厂商轨迹 | `hybrid_motion/rm_algo.py` | rm_algo_pose_move 封装（demo 可选） |\n"
        "| 扫描日志 | `hybrid_motion/scan_log.py` | pose_d vs pose_act NPZ 记录与 jerk 统计 |\n"
        "| CANFD I/O | `motion/canfd.py` | movev 初始化/速度下发/quiescence handoff |\n"
        "| Demo 入口 | `tmp/Velocity_Admittance/run_admittance.py` | 通用 YAML 驱动入口 |\n"
        "| 相位 demo | `demo/d_to_a_sin_tool_y.py` | MoveJ→D + sin_tool_y 扫描 |\n\n"
        "**与 joint_admittance（WBC 位置环）的区别：**\n"
        "- velocity：外环输出 **6D 笛卡尔速度** → `rm_movev_canfd`\n"
        "- joint_admittance：外环输出 twist → WBC QP → **关节角** → `rm_movej_canfd`\n\n"
        "`velocity_admittance/` 包仅为 deprecated 重导出，实际实现在 `hybrid_motion/`。\n\n"
        "---\n\n"
    )
    parts = [header]
