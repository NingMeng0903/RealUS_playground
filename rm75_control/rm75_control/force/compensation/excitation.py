...
def stabilize_joint_canfd_hold(
    ...
    *,
    ...
    n_frames: int = 80,
    dwell_s: float = 1.5,
    still_deg: float = 0.03,
    still_streak: int = 25,
    still_timeout_s: float = 8.0,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Hold measured joint angles via joint-CANFD until joints stop moving, then dwell.
    ...
    Re-sends encoder q each tick (not a single frozen snapshot) and waits for a
    stillness streak before dwell.
    """
    ...
    t0 = time.monotonic()
    frames = 0
    while frames < n_frames or quiet < still_streak:
        if time.monotonic() - t0 > still_timeout_s and quiet < still_streak:
            print(
                f"  WARN: joint hold stillness timeout "
                f"(quiet={quiet}/{still_streak}, max_dq={still_deg:.3f}°/tick)",
                flush=True,
            )
            break
        ret, st = robot.rm_get_current_arm_state()
        if ret != 0:
            raise RuntimeError(f"get state during joint hold failed: {ret}")
        q = np.asarray(st["joint"][:7], dtype=float)
        pose = np.asarray(st["pose"][:6], dtype=float)
...
def begin_pose_d_vel_burst(
    ...
    *,
    ...
    quiescent_ok = bool(diag.get("quiescent_ok", False))
    if not quiescent_ok or max_step > quiescent_reject_step_mm:
        reason = "not quiet" if not quiescent_ok else f"max step {max_step:.2f}mm"
        print(
            f"  WARN: handoff quiescence ({reason}) — extended zero-vel bridge",
            flush=True,
        )
        dt_s = dt_ms / 1000.0
        ...
        from rm75_control.motion.canfd import wait_movev_quiescent

        pose_quiet, max_step, q_frames, next_tick, ok = wait_movev_quiescent(
            bot.robot,
            dt_ms=dt_ms,
            follow=False,
            settle_mm=quiescent_mm,
            need_consecutive=quiescent_consecutive,
            warmup_frames=quiescent_warmup_frames,
            reject_max_step_mm=quiescent_reject_step_mm,
            next_tick=next_tick,
            max_frames=300,
        )
        diag["quiescent_max_step_mm"] = max_step
        diag["quiescent_frames"] = q_frames
        diag["quiescent_ok"] = ok
        if pose_quiet is not None:
            diag["quiescent_pose"] = pose_quiet.tolist()
        if not ok:
            raise RuntimeError(
                f"movev quiescence failed after extended bridge "
                f"(max_step={max_step:.2f}mm) — aborting burst to avoid IK snap"
            )
