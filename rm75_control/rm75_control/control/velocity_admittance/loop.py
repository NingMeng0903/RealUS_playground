"""Velocity admittance control loop: external reference + force observer + movev I/O."""
...
from rm75_control.control.admittance_common.async_state import RealtimeStateObserver, create_state_observer
...
from rm75_control.control.admittance_common.reference import MotionReference, MotionReferenceSource

from .paths import CONFIG_ROBOT, PHI_JSON
from .reference_shaper import PassThroughShaper, ReferenceShaper, build_shaper
...
def _controller_scan_log_fields(controller: AdmittanceController) -> dict[str, float | bool]:
    ...
    if cfg.adaptive_ke.enabled:
        return {
            ...
            "instability_index_raw": float(controller.instability_index_raw),
            "instability_idx_raw": float(controller.instability_index_raw),
            "instability_idx_active": float(controller.instability_index),
            "v_r_z": float(controller.v_r_z),
            "update_gated": bool(controller._ke_estimator.update_gated),
            "ke_update_gated": bool(controller._ke_estimator.update_gated),
            "ke_dx_m": float(controller._ke_estimator.last_dx_m),
            "ke_df_n": float(controller._ke_estimator.last_df_n),
            "ke_update_count": int(controller._ke_estimator.update_count),
            "force_pred_z": float(controller.force_pred_z),
            "force_dot_z": float(controller.force_dot_z),
            "cap_press_z": float(controller.cap_press_z),
            "cap_retract_z": float(controller.cap_retract_z),
        }
    return {
        "ke_est": float("nan"),
...
def velocity_realign_to_pose(
    robot,
    async_obs: RealtimeStateObserver,
...
def run_hybrid_motion_loop(
    raw: dict,
    *,
    source: MotionReferenceSource | None = None,
    source_factory: Callable[[np.ndarray, Any], MotionReferenceSource] | None = None,
    shaper: ReferenceShaper | None = None,
    reference_summary: str | None = None,
    title: str = "Hybrid motion",
    ...
    if source is None and source_factory is None:
        raise ValueError("run_hybrid_motion_loop requires source or source_factory")
    ...
    dt_s = dt_ms / 1000.0
    rp_cfg = raw.get("realtime_push", {})
    udp_period_ms = float(rp_cfg.get("cycle", max(1, int(round(dt_ms / 5.0))))) * 5.0
    ...
    settle_frames = int(startup.get("settle_frames", 50))
    ...
    # Static wait before rm_set_movev_canfd_init (plus 0.3s inside exit_canfd).
    pre_movev_settle_s = float(startup.get("pre_movev_settle_s", 1.5))
    movev_quiescent_mm = float(startup.get("movev_quiescent_mm", 0.25))
    movev_quiescent_consecutive = int(startup.get("movev_quiescent_consecutive", 10))
    ...
    scan_follow_ramp_s = float(startup.get("scan_follow_ramp_s", 0.0))
    ...
    traj_demo = raw.get("trajectory_demo", raw.get("trajectory", {}))
    traj_kind = str(traj_demo.get("type", getattr(source, "kind", "external")))
    ref_shaper = shaper or build_shaper(raw)
    observer = CompensatedForceObserver.from_yaml(raw)
    controller = HybridMotionController(dt_s, ctrl_cfg)
    ...
    print(
        ...
        f"UDP feedback ~{udp_period_ms:.0f}ms",
    )
    print(f"  phi: {PHI_JSON.name}  Fz_des={desired_z:.2f} N")
    print(f"  vz cap (tool TCP): ±{ctrl_cfg.max_vz_tool_m_s * 100:.1f} cm/s")
    ref_label = reference_summary or traj_kind
    print(f"  reference: {ref_label}  kind={traj_kind}")
    ...
    print(
        f"  pre-movev: enter_movev_session "
        f"(settle={pre_movev_settle_s:.1f}s pre-init, "
        f"{settle_frames}f zero, quiesce<{movev_quiescent_mm:.2f}mm×"
        f"{movev_quiescent_consecutive}, bridge={post_handoff_zero_s:.1f}s)",
        flush=True,
    )
    if tool_hint:
    ...
    with RobotSession(config=CONFIG_ROBOT) as bot:
        ...
        if source is None:
            source = source_factory(pose0, bot.robot)
        ...
        init_settle_frames = max(50, settle_frames)
        ...
        async_obs = create_state_observer(bot.robot, raw, robot_ip=bot.ip)
        async_obs.start()
        print(
        ...
        try:
            ...
            async_obs.stop()
            raise SystemExit("RealtimeStateObserver: no pose after CANFD init")
        ...
        if need_realign:
            if realign_target_mode == "pose_slot" and pose_slot_cartesian is not None:
                ...
                # Undo CANFD init snap only — never chase stale yaml pose_base FK.
        ...
        try:
            while True:
                ...
                if pending_scan and not scan_started:
                    ...
                    source.set_origin(traj_origin)
                    ref_shaper.reset(traj_origin)
                    controller.reset(clear_velocity=ctrl_cfg.open_loop)
                    extra = ""
                    if traj_kind == "slot_transfer_sin_tool_y" and hasattr(source, "_yaml_tcp_drift_mm"):
                        drift = float(getattr(source, "_yaml_tcp_drift_mm", 0.0))
                        if drift > 5.0:
                            extra = f"  anchored @ contact (residual {drift:.1f}mm vs slot d)"
                    print(
                        f"  scan ON @ t={t_s:.1f}s  Fz={f_ext[2]:+.2f}N  traj={traj_kind}{extra}",
                        flush=True,
                    )
                ...
                if not scan_started:
                    ...
                    follow_cmd = (
                        False
                        if follow_warmup_s > 0.0 and t_s < follow_warmup_s
                        else follow
                    )
                    if hold_active:
                ...
                raw_ref = source.sample(t_scan)
                ref = ref_shaper.step(raw_ref, dt_s)
                if not ref.valid:
                    ref = MotionReference.from_pose_hold(pose)
                pose_d_log = ref.pose_d.copy()
                vel_ff_log = ref.vel_ff.copy()
                v_cmd = controller.compute_velocity_command(
                    pose, ref.pose_d, ref.vel_ff, f_ext, f_des,
                    in_contact=True,
                ...
                scan_follow = (
                    False
                    if scan_follow_ramp_s > 0.0 and t_scan < scan_follow_ramp_s
                    else follow
                )
                send_velocity_canfd(
                    bot.robot, v_cmd.tolist(),
                    follow=scan_follow, trajectory_mode=traj_mode, radio=radio,
                )
...
def run_velocity_admittance(raw: dict, **kwargs) -> int:
    """Deprecated — prefer run_hybrid_motion_loop with an explicit MotionReferenceSource."""
    if kwargs.get("source") is None and kwargs.get("source_factory") is None:
        if raw.get("trajectory_demo") or raw.get("trajectory"):
            import sys

            from rm75_control.force.compensation.paths import REPO

            va_dir = REPO / "tmp" / "Velocity_Admittance"
            if str(va_dir) not in sys.path:
                sys.path.insert(0, str(va_dir))
            from demo_stack import (
                build_demo_shaper,
                build_demo_source_factory,
                demo_trajectory_summary,
            )

            kwargs.setdefault("source_factory", build_demo_source_factory(raw))
            kwargs.setdefault("shaper", build_demo_shaper(raw))
            kwargs.setdefault("reference_summary", demo_trajectory_summary(raw))
    return run_hybrid_motion_loop(raw, **kwargs)
