...
def compute_layout(spec: dict[str, Any]) -> dict[str, float]:
    ...
    overhead = float(rail["end_overhead_mm"]) * m
    rail_len_y = travel + overhead
...
def build_urdf_string(spec: dict | str | Path) -> str:
    ...
    slider_link = (
        ...
        f'    <origin xyz="0 0 {slider_joint_z:.6f}" rpy="0 0 0" />\n'
        '    <parent link="rail_link" />\n'
        '    <child link="slider_link" />\n'
        '    <axis xyz="0 1 0" />\n'
        f'    <limit lower="{-rail_y_limit:.6f}" upper="{rail_y_limit:.6f}" '
