...
class RailExtensionTask:
    ...
    def _soft_travel(self) -> tuple[float, float]:
        ...
        return soft_rail_travel(
            float(self.kin.q_lower[RAIL_INDEX]),
            float(self.kin.q_upper[RAIL_INDEX]),
            float(self.cfg.soft_min_m),
            float(self.cfg.soft_max_m),
        )
    ...
    def _leave_margin_m(self) -> float:
        return leave_margin_m(self.cfg.escape_leave_m, self.cfg.pin_margin_m)
    ...
    def _policy_escape_sign(self, q_rail: float | None = None) -> float:
        ...
        lo, hi = self._soft_travel()
        return policy_escape_sign(
            raw,
            y,
            lo,
            hi,
            latched_sign=float(self._escape_sign),
        )
    ...
    def _in_leave_band(self, q_rail: float, sign: float = 0.0) -> bool:
        ...
        return in_leave_band(q_rail, lo, hi, self._leave_margin_m(), s)
    ...
    def _call_reach(
        ...
        *,
        ...
        if block_escape and not allow_press_escape:
            ...
            if not self._escape_active or abs(float(self._escape_sign)) < 1.0e-12:
                self._escape_sign = self._policy_escape_sign(y)
                self._escape_active = abs(float(self._escape_sign)) > 1.0e-12
            v_escape = 0.25 * float(self.cfg.k_esc) * float(self._escape_sign)
        v_escape = float(
        ...
        if abs(v_escape) <= 1.0e-12:
            if not allow_press_escape:
                self._clear_escape_latch()
            elif abs(float(self._escape_sign)) <= 1.0e-12:
                self._escape_active = False
        v = float(v_escape)
        if use_limiters:
    ...
    def __call__(
        ...
        *,
        ...
        return self._call_reach(
            ...
            sigma_raw=sigma_raw,
            press_escape_allowed=press_escape_allowed,
