...
def test_replay_ablation_flags_disable_clamp_jerk_and_secondary(tmp_path: Path) -> None:
    source = tmp_path / "small_wbc.csv"
    _write_fixture(source)
    result = replay_csv(
        source,
        CONFIG,
        disable_cbf=True,
        disable_step_clamp=True,
        disable_jerk_box=True,
        disable_secondary=True,
        mode="free-running",
    )
    summary = result["summary"]
    assert summary["post_qp_step_clamp"] is False
    assert summary["jerk_box_enabled"] is False
    assert summary["secondary_enabled"] is False
    assert summary["qp1_status_counts"] == {"solved": 3}


def test_replay_can_force_legacy_step_clamp(tmp_path: Path) -> None:
    source = tmp_path / "small_wbc.csv"
    _write_fixture(source)
    result = replay_csv(
        source,
        CONFIG,
        disable_cbf=True,
        enable_step_clamp=True,
        mode="free-running",
    )
    assert result["summary"]["post_qp_step_clamp"] is True


def test_replay_qmeas_filter_hold_uses_logged_state(tmp_path: Path) -> None:
    source = tmp_path / "small_wbc.csv"
    _write_fixture(source)
    result = replay_csv(
        source,
        ...
        mode="free-running",
        qmeas_filter="hold",
        use_logged_qmeas=True,
    )
    assert result["summary"]["qmeas_filter"] == "hold"
    assert result["summary"]["use_logged_qmeas"] is True


def test_ablate_fs4_spectrum_finds_quarter_sample_peak() -> None:
    from apps.joint_admittance_8dof.ablate_fs4 import _accel_peak_near_fs4

    dt = 0.005
    t = np.arange(256) * dt
    qdot = np.sin(2.0 * np.pi * 50.0 * t)
    peak = _accel_peak_near_fs4(qdot, dt)
    assert peak["fs4_hz"] == pytest.approx(50.0)
    assert abs(peak["peak_hz"] - 50.0) < 3.0
