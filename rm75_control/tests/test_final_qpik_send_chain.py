...
# Use the calibrated design-family pose so direction-preserving QP tests do
# exercise the send chain rather than its intentional infeasibility pause.
Q_SAFE = np.array(
...
def _ideal_update(controller: JointIkController, twist: np.ndarray, *args, **kwargs):
    """Advance the offline fixture with explicit rail feedback/timing."""
    kwargs.setdefault("rail_exec_vel_m_s", float(controller.core.qdot_prev[0]))
    kwargs.setdefault("dt_wall_s", float(controller.cfg.dt))
    return controller.update(twist, *args, **kwargs)
...
def test_backend_failure_rejects_publication_and_keeps_history() -> None:
    controller = _controller()
    ...
    assert step.task_paused
    np.testing.assert_allclose(controller.q_cmd, q_before, atol=1e-12, rtol=0.0)
...
def test_cartesian_qpik_rejects_missing_measured_state() -> None:
    ...
    with np.testing.assert_raises_regex(ValueError, "q_meas is required"):
        _ideal_update(controller, np.zeros(6))
...
def test_rail_panic_is_reported_before_not_armed() -> None:
    ...
    events: list[str] = []
    accepted, reason = _reserve_rail_target(
...
def test_uncertified_brake_guard_precedes_all_normal_tick_publication() -> None:
    ...
    rail_reserve = source.index("_reserve_rail_target(", inner_update)
    arm_publish = source.index("_send_joint_canfd_cmd(", inner_update)
    rail_commit = source.index("commit_reservation", inner_update)
    assert guard < inner_update < rail_reserve < arm_publish < rail_commit
...
def test_zero_v_cmd_does_not_invent_rail_task() -> None:
    ...
    step = _ideal_update(controller, np.zeros(6), q_meas=q)
...
def test_lead_clamp_does_not_invent_qdot_above_vmax() -> None:
    ...
    step = _ideal_update(controller, twist, q_meas=q_meas)
...
def test_nonzero_v_cmd_publishes_qpik_qdot_not_v_ff_rail() -> None:
    ...
    step = None
    for _ in range(40):
        step = _ideal_update(controller, twist, q_meas=controller.q_cmd.copy(), vel_ff=twist)
