...
apps/joint_admittance/d_sin_tool_y.py instead.

The outer admittance loop emits a Cartesian twist; the inner WBC slack-QP loop turns
...
def main() -> int:
    ...
    print(
        f"Inner loop: WBC slack-QP control_frame={inner_cfg.control_frame} "
        f"dt={dt*1000:.0f}ms v_scale={inner_cfg.v_scale} "
        f"collision={inner_cfg.qp.collision.enabled}",
        flush=True,
    )
