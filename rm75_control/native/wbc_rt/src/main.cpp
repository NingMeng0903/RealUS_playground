  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--config" && i + 1 < argc) config_path = argv[++i];
      ...
      std::cout << "version " << wbc_rt::kVersion
                ...
                << " out " << sizeof(wbc_rt::WbcOut) << " capabilities rocking_interval_v1 command_power_v1 command_twist_v1\n";
      return 0;
    } else if (a == "--hash") {
  ...
  try {
    ...
    while (!g_stop) {
      ...
      if (cmd == wbc_rt::kCmdEnable) {
        ...
        if (in->command_twist_count > 16) throw std::invalid_argument("too many command twist rows");
        tin.command_twist_count = in->command_twist_count;
        for (int i = 0; i < 16; ++i) {
          tin.command_twist_lower[i] = in->command_twist_lower[i];
          tin.command_twist_upper[i] = in->command_twist_upper[i];
          for (int j = 0; j < 6; ++j) tin.command_twist_rows_base(i,j) = in->command_twist_rows_base[i][j];
        }
