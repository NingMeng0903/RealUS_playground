...
Config Config::load(const std::string& path) {
  ...
  while (std::getline(in, line)) {
    ...
    if (key == "urdf") c.urdf = rest;
    ...
    else if (key == "nullspace.homotopy_fade_s") c.ns_homotopy_fade_s = n0();
      ...
      c.have_flange_tcp = true;
    } else if (key == "tcp_placement_R") {
      if (nums.size() >= 9) {
        for (int r = 0; r < 3; ++r)
          for (int col = 0; col < 3; ++col) c.R_tcp_placement(r, col) = nums[r * 3 + col];
        c.have_tcp_placement = true;
      }
    } else if (key == "tcp_placement_t") {
      fill_vec(nums, c.t_tcp_placement.data(), 3);
      c.have_tcp_placement = true;
