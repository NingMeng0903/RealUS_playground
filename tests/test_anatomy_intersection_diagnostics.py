...
def test_tube_rest_acceptance_restores_whole_harmonic_mesh(monkeypatch) -> None:
    class Fixture(SimpleNamespace):
        def validate(self) -> None:
            return None
    ...
    accepted, report = (
        intersection_diagnostics.enforce_tube_rest_intersection_nonregression(asset)
    )

    np.testing.assert_array_equal(accepted.vertices_rest[:3], harmonic[:3])
    assert report["rejected_mesh_count"] == 1
    assert report["meshes"]["Artery"]["accepted"] is False
